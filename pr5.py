...
def namdz_delay(p):
    while(p>1):
            p=p-1
            print(f'[NAMDZ][|][LO......][{p}]','     ',end='\r');sleep(1/6)
            print(f'[NAMDZ][/][LOA.....][{p}]','     ',end='\r');sleep(1/6)
            print(f'[NAMDZ][-][LOAD....][{p}]','     ',end='\r');sleep(1/6)
            print(f'[NAMDZ][+][LOADI...][{p}]','     ',end='\r');sleep(1/6)
            print(f'[NAMDZ][\\][LOADIN..][{p}]','     ',end='\r');sleep(1/6)
            print(f'[NAMDZ][|][LOADING.][{p}]','     ',end='\r');sleep(1/6)
def getthongtinfacebook(cookie: str):
    ...
    try:
        print(f'\033[0;31m🐕 Đang Tiến Hành Check Live 🐕', end="\r")
...
def UpAvt(self, cookie, id_page, link_anh):
    ...
    try:
        ...
        return False
def RegPage(cookie, name, uid, fb_dtsg, jazoest):
    ...
    try:
        ...
        print('\033[1;36m▶ \033[1;35mReg Thất Bại Có Vẻ Acc Của Bạn Đã Bị Block!!')
        return False
# =========================== [ START TOOL ] ===========================
while True:
    stt+=1
    cookie_fb = input(f'\033[1;36m▶ \033[1;36mVUI LÒNG NHẬP COOKIE THỨ {stt}:\033[0;32m ')
    if cookie_fb == '':
        break
    checklive = getthongtinfacebook(cookie_fb)
    if checklive != False:
        print('\033[1;33m🐅 Name Facebook 🐅: \033[1;33m'+checklive[0] + '\033[1;33m')
...
print('\033[1;36m─'*50)
luachon = input('\033[1;36m▶ \033[1;36mBẠN MUỐN REG PAGE XONG UP AVT KHÔNG? [Y/N]: ')
if luachon == 'Y' or luachon == 'y':
    ...
    pass
print('\033[1;36m─'*50)
slpage = int(input('\033[1;36m▶ \033[1;36mBẠN MUỐN TẠO BAO NHIÊU PAGE THÌ DỪNG TOOL: '))
print('\033[1;36m─'*50)
delay = int(input('\033[1;36m▶ \033[1;36mVUI LÒNG NHẬP DELAY REG PAGE: '))
if delay < 300:
    while True:
        try:
            print('🐇 VUI LÒNG NHẬP DELAY LỚN HƠN 300')
            delay = int(input('\033[1;36m▶ \033[1;36mVUI LÒNG NHẬP DELAY REG PAGE: '))
            if delay >= 300:
                break
...
while True:
    for dulieuclone in list_clone:
        idpage = RegPage(str(dulieuclone).split('|')[0], str(dulieuclone).split('|')[1], str(dulieuclone).split('|')[2], str(dulieuclone).split('|')[3], str(dulieuclone).split('|')[4])
        if luachon == 'Y' or luachon == 'y':
            link_anh = random.choice(list_img)
            UpAvt(str(dulieuclone).split('|')[0], idpage, link_anh)
            if dpcutevcl != False:
                print(f'💤 \033[1;34mUP AVT SUCCESS | [UID PAGE: {idpage}]')
            else:
                print(f'💤 \033[1;34mUP AVT ERROR | [UID PAGE: {idpage}]')
        namdz_delay(delay)
        if dem == slpage:
            input(f'🦈 \033[1;34mDone {dem}, 🦈 \033[1;34mPage </> ENTER ĐỂ THOÁT')
            exit()